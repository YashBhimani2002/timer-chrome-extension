    ...
    return (
        <div className="bg-gray-800 w-52 h-full p-2">
            <div className="flex flex-1 items-center mt-2 mb-3.5">
                <span className=" text-white text-xl flex flex-1 w-full justify-center space-x-2">
                    <p className="bg-gray-600 px-1 py-0.5">{time.hours < 10 ? "0" + time.hours : time.hours}</p>
                    <p>:</p>
                    <p className="bg-gray-600 px-1 py-0.5">{time.minutes < 10 ? "0" + time.minutes : time.minutes}</p>
                    <p>:</p>
                    <p className="bg-gray-600 px-1 py-0.5">{time.seconds < 10 ? "0" + time.seconds : time.seconds}</p></span>
            ...
            <div className="flex flex-col justify-center items-center gap-2">
                <div className="flex gap-2 w-full">
                    ...
                    <input type="time" id="readTime" name="readTime" value={timerValues.readTime} min="00:00" max="59:59" step="1" className="flex-1 self-start px-2 border-[1px] rounded-sm" onChange={(e) => {
                ...
                <div className="flex gap-2 w-full">
                    ...
                    <input type="time" id="breakTime" name="breakTime" value={timerValues.breakTime}
                        min="0:00" max="59:59" step="1" className="flex-1 self-start px-2 border-[1px] rounded-sm" onChange={(e) => {
                            setTimerValues({ ...timerValues, breakTime: e.target.value })
                        }} />
                </div>
            </div>
            <div className="flex gap-2 my-2">
                <button className={`${time.hours == 0 && time.minutes == 0 && time.seconds == 0 ? "bg-green-500 hover:bg-green-300" : "bg-yellow-300 hover:bg-yellow-200"}  rounded-[4px] border-transparent border-[1px] shadow-md h-8 w-full font-bold`} onClick={() => handleStartTimer(time.hours == 0 && time.minutes == 0 && time.seconds == 0 ? "start" : "resume")}>{time.hours == 0 && time.minutes == 0 && time.seconds == 0 ? "Start" : "Resume"}</button>

                <button className=" bg-red-500 hover:bg-red-300 rounded-[4px] border-transparent border-[1px] shadow-md h-8 w-full font-bold" onClick={() => handleStartTimer("stop")}>Stop</button>
            </div>
